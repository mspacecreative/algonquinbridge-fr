...
else
{
	if(isset($_REQUEST["param"]))
	{
		...
		else if($_REQUEST["param"] == "save_number_control")
		{
			$dynamic_Id = intval($_REQUEST["ux_hd_textbox_dynamic_id"]);
			$form_id = intval($_REQUEST["form_id"]);
			$event = esc_attr($_REQUEST["event"]);
			...
			$form_settings = isset($_REQUEST["form_settings"]) ? json_decode(stripcslashes($_REQUEST["form_settings"]),true) : array();
			$form_settings[$dynamic_Id]["dynamic_id"] = $dynamic_Id;
			$form_settings[$dynamic_Id]["control_type"] = "7";
			$form_settings[$dynamic_Id]["cb_label_value"] = isset($_REQUEST["ux_label_text_".$dynamic_Id]) ? esc_attr($_REQUEST["ux_label_text_".$dynamic_Id]) : "Number";
			$form_settings[$dynamic_Id]["cb_description"] = isset($_REQUEST["ux_description_control_".$dynamic_Id]) ? esc_attr($_REQUEST["ux_description_control_".$dynamic_Id]) : "";
			$form_settings[$dynamic_Id]["cb_control_required"] = isset($_REQUEST["ux_required_control_radio_".$dynamic_Id]) ? esc_attr($_REQUEST["ux_required_control_radio_".$dynamic_Id]) : "0";
			$form_settings[$dynamic_Id]["cb_tooltip_txt"] = isset($_REQUEST["ux_tooltip_control_".$dynamic_Id]) ? esc_attr($_REQUEST["ux_tooltip_control_".$dynamic_Id]) : "";
			$form_settings[$dynamic_Id]["cb_default_txt_val"] = isset($_REQUEST["ux_default_value_".$dynamic_Id]) ?  esc_attr($_REQUEST["ux_default_value_".$dynamic_Id]) : "";
			$form_settings[$dynamic_Id]["cb_admin_label"] = isset($_REQUEST["ux_admin_label_".$dynamic_Id]) ?  esc_attr($_REQUEST["ux_admin_label_".$dynamic_Id]) : "Number";
			$form_settings[$dynamic_Id]["cb_show_email"] = isset($_REQUEST["ux_show_email_".$dynamic_Id]) ? "1" : "0";
			foreach($form_settings as $element)
			{
				$id = $element["dynamic_id"];
		...
		else if($_REQUEST["param"] == "save_name_control")
		{
			$dynamic_Id = intval($_REQUEST["ux_hd_textbox_dynamic_id"]);
			$form_id = intval($_REQUEST["form_id"]);
			$event = esc_attr($_REQUEST["event"]);
			...
			$form_settings = isset($_REQUEST["form_settings"]) ? json_decode(stripcslashes($_REQUEST["form_settings"]),true) : array();
			$form_settings[$dynamic_Id]["dynamic_id"] = $dynamic_Id;
			$form_settings[$dynamic_Id]["control_type"] = "8";
			$form_settings[$dynamic_Id]["cb_label_value"] = isset($_REQUEST["ux_label_text_".$dynamic_Id]) ? esc_attr($_REQUEST["ux_label_text_".$dynamic_Id]): "Name";
			$form_settings[$dynamic_Id]["cb_description"] = isset($_REQUEST["ux_description_control_".$dynamic_Id]) ? esc_attr($_REQUEST["ux_description_control_".$dynamic_Id]) : "";
			$form_settings[$dynamic_Id]["cb_control_required"] = isset($_REQUEST["ux_required_control_radio_".$dynamic_Id]) ? esc_attr($_REQUEST["ux_required_control_radio_".$dynamic_Id]) : "0";
			$form_settings[$dynamic_Id]["cb_tooltip_txt"] = isset($_REQUEST["ux_tooltip_control_".$dynamic_Id]) ? esc_attr($_REQUEST["ux_tooltip_control_".$dynamic_Id]) : "";
			$form_settings[$dynamic_Id]["cb_default_txt_val"] = isset($_REQUEST["ux_default_value_".$dynamic_Id]) ? esc_attr($_REQUEST["ux_default_value_".$dynamic_Id]) : "";
			$form_settings[$dynamic_Id]["cb_admin_label"] = isset($_REQUEST["ux_admin_label_".$dynamic_Id]) ? esc_attr($_REQUEST["ux_admin_label_".$dynamic_Id]) : "Name";
			$form_settings[$dynamic_Id]["cb_show_email"] = isset($_REQUEST["ux_show_email_".$dynamic_Id]) ? "1" : "0";
			foreach($form_settings as $element)
			{
				$id = $element["dynamic_id"];
		...
		else if($_REQUEST["param"] == "save_file_upload_control")
		{
			$dynamic_Id = intval($_REQUEST["ux_hd_textbox_dynamic_id"]);
			$form_id = intval($_REQUEST["form_id"]);
			$event = esc_attr($_REQUEST["event"]);
			...
			$form_settings = isset($_REQUEST["form_settings"]) ? json_decode(stripcslashes($_REQUEST["form_settings"]),true) : array();
			$form_settings[$dynamic_Id]["dynamic_id"] = $dynamic_Id;
			$form_settings[$dynamic_Id]["control_type"] = "9";
			$form_settings[$dynamic_Id]["cb_label_value"] = isset($_REQUEST["ux_label_text_".$dynamic_Id]) ? esc_attr($_REQUEST["ux_label_text_".$dynamic_Id]) : "File Upload";
			$form_settings[$dynamic_Id]["cb_description"] = isset($_REQUEST["ux_description_control_".$dynamic_Id]) ? esc_attr($_REQUEST["ux_description_control_".$dynamic_Id]) : "";
			$form_settings[$dynamic_Id]["cb_tooltip_txt"] = isset($_REQUEST["ux_tooltip_control_".$dynamic_Id]) ? esc_attr($_REQUEST["ux_tooltip_control_".$dynamic_Id]) : "";
			$form_settings[$dynamic_Id]["cb_admin_label"] =  isset($_REQUEST["ux_admin_label_".$dynamic_Id]) ? esc_attr($_REQUEST["ux_admin_label_".$dynamic_Id]): "File Upload";
			$form_settings[$dynamic_Id]["cb_show_email"] = isset($_REQUEST["ux_show_email_".$dynamic_Id]) ? "1" : "0";
			$form_settings[$dynamic_Id]["cb_allow_multiple_file"] = isset($_REQUEST["ux_allow_multiple_file_".$dynamic_Id]) ? "1" : "0";
			$form_settings[$dynamic_Id]["cb_control_required"] = "0";
			$file_upload_extensions =  isset($_REQUEST["ux_allowed_file_extensions_".$dynamic_Id]) ? esc_attr($_REQUEST["ux_allowed_file_extensions_".$dynamic_Id]) : "jpg,png,gif,jpeg";
			$form_settings[$dynamic_Id]["cb_allow_file_ext_upload"] = $file_upload_extensions;
			$file_upload_file_size = isset($_REQUEST["ux_maximum_file_allowed_".$dynamic_Id]) ? esc_attr($_REQUEST["ux_maximum_file_allowed_".$dynamic_Id]) : "10";
			$form_settings[$dynamic_Id]["cb_maximum_file_allowed"] = $file_upload_file_size;
			foreach($form_settings as $element)
			{
				$id = $element["dynamic_id"];
				$control_type = $element["control_type"];
		...
		else if($_REQUEST["param"] == "save_phone_control")
		{
			$dynamic_Id = intval($_REQUEST["ux_hd_textbox_dynamic_id"]);
			$form_id = intval($_REQUEST["form_id"]);
			$event = esc_attr($_REQUEST["event"]);
			...
			$form_settings = isset($_REQUEST["form_settings"]) ? json_decode(stripcslashes($_REQUEST["form_settings"]),true) : array();
			$form_settings[$dynamic_Id]["dynamic_id"] = $dynamic_Id;
			$form_settings[$dynamic_Id]["control_type"] = "10";
			$form_settings[$dynamic_Id]["cb_label_value"] = isset($_REQUEST["ux_label_text_".$dynamic_Id]) ? esc_attr($_REQUEST["ux_label_text_".$dynamic_Id]) : "Phone";
			$form_settings[$dynamic_Id]["cb_description"] = isset($_REQUEST["ux_description_control_".$dynamic_Id]) ? esc_attr($_REQUEST["ux_description_control_".$dynamic_Id]) : "";
			$form_settings[$dynamic_Id]["cb_control_required"] = isset($_REQUEST["ux_required_control_radio_".$dynamic_Id]) ? esc_attr($_REQUEST["ux_required_control_radio_".$dynamic_Id]): "0" ;
			$form_settings[$dynamic_Id]["cb_tooltip_txt"] = isset($_REQUEST["ux_tooltip_control_".$dynamic_Id]) ? esc_attr($_REQUEST["ux_tooltip_control_".$dynamic_Id]) : "";
			$form_settings[$dynamic_Id]["cb_default_txt_val"] = isset($_REQUEST["ux_default_value_".$dynamic_Id]) ? esc_attr($_REQUEST["ux_default_value_".$dynamic_Id]) : "";
			$form_settings[$dynamic_Id]["cb_admin_label"] = isset($_REQUEST["ux_admin_label_".$dynamic_Id]) ? esc_attr($_REQUEST["ux_admin_label_".$dynamic_Id]) : "Phone";
			$form_settings[$dynamic_Id]["cb_show_email"] = isset($_REQUEST["ux_show_email_".$dynamic_Id]) ? "1" : "0";
		...
		else if($_REQUEST["param"] == "save_address_control")
		{
			$dynamic_Id = intval($_REQUEST["ux_hd_textbox_dynamic_id"]);
			$form_id = intval($_REQUEST["form_id"]);
			$event = esc_attr($_REQUEST["event"]);
			...
			$form_settings = isset($_REQUEST["form_settings"]) ? json_decode(stripcslashes($_REQUEST["form_settings"]),true) : array();
			$form_settings[$dynamic_Id]["dynamic_id"] = $dynamic_Id;
			$form_settings[$dynamic_Id]["control_type"] = "11";
			$form_settings[$dynamic_Id]["cb_label_value"] = isset($_REQUEST["ux_label_text_".$dynamic_Id]) ? esc_attr($_REQUEST["ux_label_text_".$dynamic_Id]) : "Address";
			$form_settings[$dynamic_Id]["cb_description"] = isset($_REQUEST["ux_description_control_".$dynamic_Id]) ? esc_attr($_REQUEST["ux_description_control_".$dynamic_Id]) : "";
			$form_settings[$dynamic_Id]["cb_control_required"] = isset($_REQUEST["ux_required_control_radio_".$dynamic_Id]) ? esc_attr($_REQUEST["ux_required_control_radio_".$dynamic_Id]) : "0";
			$form_settings[$dynamic_Id]["cb_tooltip_txt"] = isset($_REQUEST["ux_tooltip_control_".$dynamic_Id]) ? esc_attr($_REQUEST["ux_tooltip_control_".$dynamic_Id]) : "";
			$form_settings[$dynamic_Id]["cb_default_txt_val"] = isset($_REQUEST["ux_default_value_".$dynamic_Id]) ? esc_attr($_REQUEST["ux_default_value_".$dynamic_Id]): "";
			$form_settings[$dynamic_Id]["cb_admin_label"] = isset($_REQUEST["ux_admin_label_".$dynamic_Id]) ? esc_attr($_REQUEST["ux_admin_label_".$dynamic_Id]) : "Address";
			$form_settings[$dynamic_Id]["cb_show_email"] = isset($_REQUEST["ux_show_email_".$dynamic_Id]) ? "1" : "0";
			foreach($form_settings as $element)
			{
				$id = $element["dynamic_id"];
		...
		else if($_REQUEST["param"] == "save_date_control")
		{
			$dynamic_Id = intval($_REQUEST["ux_hd_textbox_dynamic_id"]);
			$form_id = intval($_REQUEST["form_id"]);
			$event = esc_attr($_REQUEST["event"]);
			...
			$form_settings = isset($_REQUEST["form_settings"]) ? json_decode(stripcslashes($_REQUEST["form_settings"]),true) : array();
			$form_settings[$dynamic_Id]["dynamic_id"] = $dynamic_Id;
			$form_settings[$dynamic_Id]["control_type"] = "12";
			$form_settings[$dynamic_Id]["cb_label_value"] = isset($_REQUEST["ux_label_text_".$dynamic_Id]) ? esc_attr($_REQUEST["ux_label_text_".$dynamic_Id]) : "Date";
			$form_settings[$dynamic_Id]["cb_description"] = isset($_REQUEST["ux_description_control_".$dynamic_Id]) ? esc_attr($_REQUEST["ux_description_control_".$dynamic_Id]) : "";
			$form_settings[$dynamic_Id]["cb_control_required"] = isset($_REQUEST["ux_required_control_radio_".$dynamic_Id]) ? esc_attr($_REQUEST["ux_required_control_radio_".$dynamic_Id]) : "0";
			$form_settings[$dynamic_Id]["cb_tooltip_txt"] = isset($_REQUEST["ux_tooltip_control_".$dynamic_Id]) ? esc_attr($_REQUEST["ux_tooltip_control_".$dynamic_Id]) : "";
			$form_settings[$dynamic_Id]["cb_admin_label"] = isset($_REQUEST["ux_admin_label_".$dynamic_Id]) ? esc_attr($_REQUEST["ux_admin_label_".$dynamic_Id]) : "Date";
			$form_settings[$dynamic_Id]["cb_show_email"] = isset($_REQUEST["ux_show_email_".$dynamic_Id]) ? "1" : "0";
			$form_settings[$dynamic_Id]["cb_start_year"] = isset($_REQUEST["ux_start_year_label_".$dynamic_Id]) ? esc_attr($_REQUEST["ux_start_year_label_".$dynamic_Id]) : "1900";
			$form_settings[$dynamic_Id]["cb_end_year"] = isset($_REQUEST["ux_last_year_label_".$dynamic_Id]) ? esc_attr($_REQUEST["ux_last_year_label_".$dynamic_Id]) : "2100";
			$form_settings[$dynamic_Id]["cb_default_value_day"] = isset($_REQUEST["ux_default_day_type_".$dynamic_Id]) ? esc_attr($_REQUEST["ux_default_day_type_".$dynamic_Id]) : date("j");
			$form_settings[$dynamic_Id]["cb_default_value_month"] = isset($_REQUEST["ux_default_month_type_".$dynamic_Id]) ? esc_attr($_REQUEST["ux_default_month_type_".$dynamic_Id]) : date("n");
			$form_settings[$dynamic_Id]["cb_default_value_year"] = isset($_REQUEST["ux_default_year_type_".$dynamic_Id]) ? esc_attr($_REQUEST["ux_default_year_type_".$dynamic_Id]) : date("Y");
			$form_settings[$dynamic_Id]["cb_date_format"] = isset($_REQUEST["uxDefaultDateFormat_".$dynamic_Id]) ? esc_attr($_REQUEST["uxDefaultDateFormat_".$dynamic_Id]) : "0";
			foreach($form_settings as $element)
			{
				$id = $element["dynamic_id"];
				$control_type = $element["control_type"];
		...
		else if($_REQUEST["param"] == "save_time_control")
		{
			$dynamic_Id = intval($_REQUEST["ux_hd_textbox_dynamic_id"]);
			$form_id = intval($_REQUEST["form_id"]);
			$event = esc_attr($_REQUEST["event"]);
			...
			$form_settings = isset($_REQUEST["form_settings"]) ? json_decode(stripcslashes($_REQUEST["form_settings"]),true) : array();
			$form_settings[$dynamic_Id]["dynamic_id"] = $dynamic_Id;
			$form_settings[$dynamic_Id]["control_type"] = "13";
			$form_settings[$dynamic_Id]["cb_label_value"] = isset($_REQUEST["ux_label_text_".$dynamic_Id]) ? esc_attr($_REQUEST["ux_label_text_".$dynamic_Id]) : "Time";
			$form_settings[$dynamic_Id]["cb_description"] = isset($_REQUEST["ux_description_control_".$dynamic_Id]) ? esc_attr($_REQUEST["ux_description_control_".$dynamic_Id]) : "";
			$form_settings[$dynamic_Id]["cb_control_required"] = isset($_REQUEST["ux_required_control_radio_".$dynamic_Id]) ? esc_attr($_REQUEST["ux_required_control_radio_".$dynamic_Id]) : "0";
			$form_settings[$dynamic_Id]["cb_tooltip_txt"] = isset($_REQUEST["ux_tooltip_control_".$dynamic_Id]) ? esc_attr($_REQUEST["ux_tooltip_control_".$dynamic_Id]) : "";
			$form_settings[$dynamic_Id]["cb_admin_label"] = isset($_REQUEST["ux_admin_label_".$dynamic_Id]) ? esc_attr($_REQUEST["ux_admin_label_".$dynamic_Id]) : "Time";
			$form_settings[$dynamic_Id]["cb_show_email"] = isset($_REQUEST["ux_show_email_".$dynamic_Id]) ? "1" : "0";
			$form_settings[$dynamic_Id]["cb_hour_format"] = isset($_REQUEST["ux_drop_hour_time_".$dynamic_Id]) ? esc_attr($_REQUEST["ux_drop_hour_time_".$dynamic_Id]) : "12";
			$form_settings[$dynamic_Id]["cb_hours"] = isset($_REQUEST["ux_default_hours_12_".$dynamic_Id]) ? esc_attr($_REQUEST["ux_default_hours_12_".$dynamic_Id]) : "";
			$form_settings[$dynamic_Id]["cb_minutes"] = isset($_REQUEST["ux_default_minute_".$dynamic_Id]) ? esc_attr($_REQUEST["ux_default_minute_".$dynamic_Id]) : "";
			$form_settings[$dynamic_Id]["cb_am_pm"] = isset($_REQUEST["ux_default_am_".$dynamic_Id]) ? esc_attr($_REQUEST["ux_default_am_".$dynamic_Id]) : "0";
			$form_settings[$dynamic_Id]["cb_time_format"] = isset($_REQUEST["ux_minute_format_".$dynamic_Id]) ? esc_attr($_REQUEST["ux_minute_format_".$dynamic_Id]) : "1";
			foreach($form_settings as $element)
			{
				$id = $element["dynamic_id"];
				$control_type = $element["control_type"];
		...
		else if($_REQUEST["param"] == "save_password_control")
		{
			$dynamic_Id = intval($_REQUEST["ux_hd_textbox_dynamic_id"]);
			$form_id = intval($_REQUEST["form_id"]);
			$event = esc_attr($_REQUEST["event"]);
			...
			$form_settings = isset($_REQUEST["form_settings"]) ? json_decode(stripcslashes($_REQUEST["form_settings"]),true) : array();
			$form_settings[$dynamic_Id]["dynamic_id"] = $dynamic_Id;
			$form_settings[$dynamic_Id]["control_type"] = "15";
			$form_settings[$dynamic_Id]["cb_label_value"] = isset($_REQUEST["ux_label_text_".$dynamic_Id]) ? esc_attr($_REQUEST["ux_label_text_".$dynamic_Id]) :"Password";
			$form_settings[$dynamic_Id]["cb_description"] = isset($_REQUEST["ux_description_control_".$dynamic_Id]) ? esc_attr($_REQUEST["ux_description_control_".$dynamic_Id]) :"";
			$form_settings[$dynamic_Id]["cb_control_required"] = isset($_REQUEST["ux_required_control_radio_".$dynamic_Id]) ? esc_attr($_REQUEST["ux_required_control_radio_".$dynamic_Id]) :"0";
			$form_settings[$dynamic_Id]["cb_tooltip_txt"] = isset($_REQUEST["ux_tooltip_control_".$dynamic_Id]) ? esc_attr($_REQUEST["ux_tooltip_control_".$dynamic_Id]) : "";
			$form_settings[$dynamic_Id]["cb_admin_label"] = isset($_REQUEST["ux_admin_label_".$dynamic_Id]) ? esc_attr($_REQUEST["ux_admin_label_".$dynamic_Id]) : "Password"; 
			$form_settings[$dynamic_Id]["cb_show_email"] = isset($_REQUEST["ux_show_email_".$dynamic_Id]) ? "1" : "0";
			$form_settings[$dynamic_Id]["cb_checkbox_alpha_filter"] = isset($_REQUEST["ux_checkbox_alpha_filter_".$dynamic_Id]) ? "1" : "0";
			$form_settings[$dynamic_Id]["cb_ux_checkbox_alpha_num_filter"] = isset($_REQUEST["ux_checkbox_alpha_num_filter_".$dynamic_Id]) ? "1" : "0";
			$form_settings[$dynamic_Id]["cb_checkbox_digit_filter"] = isset($_REQUEST["ux_checkbox_digit_filter_".$dynamic_Id]) ? "1" : "0";
			$form_settings[$dynamic_Id]["cb_checkbox_strip_tag_filter"] = isset($_REQUEST["ux_checkbox_strip_tag_filter_".$dynamic_Id]) ? "1" : "0";
			$form_settings[$dynamic_Id]["cb_checkbox_trim_filter"] = isset($_REQUEST["ux_checkbox_trim_filter_".$dynamic_Id]) ? "1" : "0";
			$form_settings[$dynamic_Id]["cb_default_txt_val"] = "Password";
			foreach($form_settings as $element)
			{
				$id = $element["dynamic_id"];
				$control_type = $element["control_type"];
		...
		else if($_REQUEST["param"] == "save_url_control")
		{
			$dynamic_Id = intval($_REQUEST["ux_hd_textbox_dynamic_id"]);
			$form_id = intval($_REQUEST["form_id"]);
			$event = esc_attr($_REQUEST["event"]);
			...
			$form_settings = isset($_REQUEST["form_settings"]) ? json_decode(stripcslashes($_REQUEST["form_settings"]),true) : array();
			$form_settings[$dynamic_Id]["dynamic_id"] = $dynamic_Id;
			$form_settings[$dynamic_Id]["control_type"] = "16";
			$form_settings[$dynamic_Id]["cb_label_value"] = isset($_REQUEST["ux_label_text_".$dynamic_Id]) ? esc_attr($_REQUEST["ux_label_text_".$dynamic_Id]): "Website / URL";
			$form_settings[$dynamic_Id]["cb_description"] = isset($_REQUEST["ux_description_control_".$dynamic_Id]) ? esc_attr($_REQUEST["ux_description_control_".$dynamic_Id]) : "";
			$form_settings[$dynamic_Id]["cb_control_required"] = isset($_REQUEST["ux_required_control_radio_".$dynamic_Id]) ? esc_attr($_REQUEST["ux_required_control_radio_".$dynamic_Id]) : "0";
			$form_settings[$dynamic_Id]["cb_tooltip_txt"] = isset($_REQUEST["ux_tooltip_control_".$dynamic_Id]) ? esc_attr($_REQUEST["ux_tooltip_control_".$dynamic_Id]) : "";
			$form_settings[$dynamic_Id]["cb_default_txt_val"] = isset($_REQUEST["ux_default_value_".$dynamic_Id]) ?  esc_attr($_REQUEST["ux_default_value_".$dynamic_Id]) : "";
			$form_settings[$dynamic_Id]["cb_admin_label"] = isset($_REQUEST["ux_admin_label_".$dynamic_Id]) ? esc_attr($_REQUEST["ux_admin_label_".$dynamic_Id]) : "Website / URL";
			$form_settings[$dynamic_Id]["cb_show_email"] = isset($_REQUEST["ux_show_email_".$dynamic_Id]) ? "1" : "0";
		...
		else if($_REQUEST["param"] == "save_captcha_control")
		{
			$dynamic_Id = intval($_REQUEST["ux_hd_textbox_dynamic_id"]);
			$form_id = intval($_REQUEST["form_id"]);
			$event = esc_attr($_REQUEST["event"]);
			...
			$form_settings = isset($_REQUEST["form_settings"]) ? json_decode(stripcslashes($_REQUEST["form_settings"]),true) : array();
			$form_settings[$dynamic_Id]["dynamic_id"] = $dynamic_Id;
			$form_settings[$dynamic_Id]["control_type"] = "17";
			$form_settings[$dynamic_Id]["cb_label_value"] = "";
			$form_settings[$dynamic_Id]["cb_description"] = "";
			$form_settings[$dynamic_Id]["cb_control_required"] =  "0";
			$form_settings[$dynamic_Id]["cb_tooltip_txt"] =  "";
			$form_settings[$dynamic_Id]["cb_default_txt_val"] = "";
			$form_settings[$dynamic_Id]["cb_admin_label"] = "";
			$form_settings[$dynamic_Id]["cb_show_email"] = "0";
